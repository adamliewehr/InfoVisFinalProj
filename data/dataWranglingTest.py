...
if __name__ == '__main__':
    # Get all files but limit to first 1000
    files = list(Path('games').glob('*.json'))[:1000]
    ...
    df.to_csv('cleaned_games_test.csv', index=False)
    ...
    print(f"\nSample winner (Rank 1) stats:")
    print(f"  Total VP: {df['rank1_total_vp'].mean():.2f}")
    print(f"  Settlements: {df['rank1_settlements'].mean():.2f}")
    print(f"  Cities: {df['rank1_cities'].mean():.2f}")
    print(f"  VP Cards: {df['rank1_vp_cards'].mean():.2f}")
    print(f"  Resources Used: {df['rank1_resourcesUsed'].mean():.2f}")
